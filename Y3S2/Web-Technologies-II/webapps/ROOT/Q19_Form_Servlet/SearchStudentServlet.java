...
public class SearchStudentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String query = "SELECT s.roll AS roll_number, s.name AS student_name, d.name AS dept_name " +
                    "FROM 1100_students AS s " +
                    "JOIN 1100_departments AS d ON s.deptID = d.id " +
                    "WHERE LOWER(s.name) LIKE LOWER(?) AND d.name = ?;";
