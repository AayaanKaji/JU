...
<form action="/Q31_Student_Result/displayMarks.jsp" method="post" accept-charset="UTF-8">
    <%
    ...
    <select name="roll">
        <%
            ...
            rs = stmt.executeQuery("SELECT * FROM 1100_students");
            while (rs.next()) {
        %>
    ...
    <select name="semester">
        <%
            stmt = conn.createStatement();
            rs = stmt.executeQuery("SELECT * FROM 1100_semesters");
            while (rs.next()) {
        %>
    ...
    <select name="subject_code">
        <%
            stmt = conn.createStatement();
            rs = stmt.executeQuery("SELECT * FROM 1100_subjects");
            while (rs.next()) {
        %>
